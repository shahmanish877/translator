...
redisClient.on('error', async (err) => {
    await logProgress('Redis Client Error', err);
});
...
const final_translate_prompt = "Translate all above inputs to hebrew language and create response in json form where the key will be the original value"
...
async function batchTranslateToHebrewGemini(textArray, prompt, retries = 10) {
    await checkCancellation();
    await logProgress("Gemini Request working on it...")
    try {
        const model = genAI.getGenerativeModel({
            model: 'gemini-1.5-flash',
            systemInstruction: prompt,
        ...
        if (error.message.includes('429 Too Many Requests') || error.message.includes('Resource has been exhausted')) {
            if (retries > 0) {
                ...
                // throw new Error('Quota limit reached. Max retries exceeded. Falling back to OpenAI.');
                await logProgress('========Bad news. Gemini Quota limit reached. Max retries exceeded. Falling back to OpenAI.');
        ...
        if (error.message.includes('blocked due to SAFETY')) {
            ...
            await logProgress(`========Bad news Unknown error, using original text - ${error.message}`);
            // return batchTranslateToHebrewOpenAI(textArray, prompt);
            return textArray; // Return the original texts if retries exceed
        }
    }
...
async function batchTranslateToHebrewGeminiVertex(textArray, prompt, retries = 10) {
    ...
    await checkCancellation();
    await logProgress("Vertex Request working on it...")
    ...
    try {
        ...
        // await logProgress("Translation Text:", translationText);
        ...
        if (error.message.includes('blocked due to SAFETY')) {
            await logProgress('Vertex AI blocked due to safety concerns. Falling back to batch openAI.');
            await delay(120000); // Wait 120 seconds
            return batchTranslateToHebrewOpenAI(textArray, prompt);
        } else {
            await logProgress(error.response);
            await logProgress(`VertexAI translation failed: ${error.message}, falling back to openAI`);
            return batchTranslateToHebrewGemini(textArray, prompt);
        }
    }
}
...
async function batchTranslateToHebrewOpenAI(textArray, prompt, retries = 10) {
    ...
    try {
        ...
        const messages = [
            ...
            {
                role: 'user',
                content: final_translate_prompt + " \n " + prompt // Include the second prompt
            }
        ];
        ...
        // await logProgress("Final prompt  = "+final_translate_prompt + " \n " + prompt)
        ...
        if (error.message.includes('429') || error.message.includes('500')) {
            if (retries > 0) {
                await delay(120000); // Wait 120 seconds
                await logProgress(`OPENAI Quota limit reached. (${retries} retries left)`);
                return batchTranslateToHebrewOpenAI(textArray, prompt, retries - 1);
            } else {
                await logProgress('OpenAI Quota limit reached. Max retries exceeded. Using gemini.');
                return batchTranslateToHebrewGeminiVertex(textArray, prompt);
                // return textArray; // Return the original texts if retries exceed
            }
        }

        await logProgress(`Unrecoverable OpenAI error: ${error.response?.data?.error?.message || error.message}. Falling to gemini vertex.`);
        return batchTranslateToHebrewGeminiVertex(textArray, prompt);
    ...
    async function traverseAndCollect(obj, path = '') {
        if (Array.isArray(obj)) {
            ...
            for (const [key, value] of Object.entries(obj)) {
                ...
                const textsToExclude = new Set([
                    // RDMA1 to RDMA12
                    ...Array.from({length: 20}, (_, i) => `RDMA${i + 1}`),

                    // P1 - RDMA5 to P3 - RDMA5
                    ...Array.from({length: 20}, (_, i) => `P${i + 1} - RDMA5`)
                ]);
                ...
                if (key === 'he'
                    ...
                    && value.trim() !== ''
                    && !isNumeric(value)
                    && !hebrewRegex.test(value)
                    && !textsToExclude.has(value.trim())
                ) {
                    try {
                    ...
                    if (translatedText) {
                        ...
                        try {
                            if(originalText.trim() == translatedText.trim()){
                                await logProgress("===Bad news, using original text for: "+originalText.trim())
                            }

                            await redisClient.set(originalText.trim(), translatedText);
                        } catch (error) {
                            await logProgress(`Redis caching error for ${path} - translation = ${translatedText}, error:`, error);
                        }
                    } else {
                        ...
                        // await delay(1000);
                        if(useGemini) {
                            singleLineTranslation = await batchTranslateToHebrewGeminiVertex(originalText, prompt);
                        }else{
                        ...
                        try {
                            if(originalText.trim() == singleLineTranslation.trim()){
                                await logProgress("===Bad news, using original text for: "+originalText.trim())
                            }
